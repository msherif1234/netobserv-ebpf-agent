...
static __always_inline void fill_flow_seq_id(flow_seq_id *seq_id, pkt_info *pkt, u32 seq,
                                             bool reverse) {
    flow_id *id = pkt->id;
    if (reverse) {
...
static __always_inline void update_reverse_flow_rtt(pkt_info *pkt, u32 seq) {
    ...
    flow_metrics *reverse_flow =
        (flow_metrics *)bpf_map_lookup_elem(&aggregated_flows, &rev_flow_id);
    if (reverse_flow != NULL) {
        if (pkt->rtt > reverse_flow->flow_rtt) {
            reverse_flow->flow_rtt = pkt->rtt;
            long ret =
                bpf_map_update_elem(&aggregated_flows, &rev_flow_id, reverse_flow, BPF_EXIST);
...
static __always_inline void __calculate_tcp_rtt(pkt_info *pkt, struct tcphdr *tcp,
                                                flow_seq_id *seq_id) {
...
static __always_inline void calculate_flow_rtt_tcp(pkt_info *pkt, u8 direction, void *data_end,
                                                   flow_seq_id *seq_id) {
